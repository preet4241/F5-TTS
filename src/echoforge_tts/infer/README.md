...
The cli command `echoforge_infer-gradio` equals to `python src/echoforge_tts/infer/infer_gradio.py`, which launches a Gradio APP (web interface) for inference.
...
echoforge_infer-gradio --inbrowser
...
echoforge_infer-gradio --root_path "/myapp"
```

Could also be used as a component for larger application:
```python
import gradio as gr
from echoforge_tts.infer.infer_gradio import app
...
The cli command `echoforge_infer-cli` equals to `python src/echoforge_tts/infer/infer_cli.py`, which is a command line tool for inference.
...
echoforge_infer-cli --model F5TTS_Base --vocoder_name bigvgan --load_vocoder_from_local

# Use custom path checkpoint, e.g.
echoforge_infer-cli --ckpt_file ckpts/F5TTS_v1_Base/model_1250000.safetensors

# More instructions
echoforge_infer-cli --help
```
...
For example, you can use `.toml` to pass in variables, refer to `src/echoforge_tts/infer/examples/basic/basic.toml`:
...
You can also leverage `.toml` file to do multi-style generation, refer to `src/echoforge_tts/infer/examples/multi/story.toml`.
...
ref_text = ""
```
You should mark the voice with `[main]` `[town]` `[country]` whenever you want to change voice, refer to `src/echoforge_tts/infer/examples/multi/story.txt`.
...
wav, sr, spec = f5tts.infer(
    ...
    file_wave=str(files("echoforge_tts").joinpath("../../tests/api_out.wav")),
    file_spec=str(files("echoforge_tts").joinpath("../../tests/api_out.png")),
    seed=None,
)
```
...
```bash
# Start socket server
python src/echoforge_tts/socket_server.py
...
python src/echoforge_tts/socket_client.py
```
...
```bash
python src/echoforge_tts/infer/speech_edit.py
```
