...
from echoforge_tts.infer.utils_infer import (
    cfg_strength,
    cross_fade_duration,
    device,
    fix_duration,
...
parser.add_argument(
    ...
    "--config",
    type=str,
    default=os.path.join(files("echoforge_tts").joinpath("infer/examples/basic"), "basic.toml"),
...
parser.add_argument(
    ...
    "--model_cfg",
    type=str,
    help="The path to EchoForge TTS model config file .yaml",
)
parser.add_argument(
    "-p",
    "--ckpt_file",
...
if "infer/examples/" in ref_audio:
    ref_audio = str(files("echoforge_tts").joinpath(f"{ref_audio}"))
if "infer/examples/" in gen_file:
    gen_file = str(files("echoforge_tts").joinpath(f"{gen_file}"))
if "voices" in config:
    for voice in config["voices"]:
        voice_ref_audio = config["voices"][voice]["ref_audio"]
        if "infer/examples/" in voice_ref_audio:
            config["voices"][voice]["ref_audio"] = str(files("echoforge_tts").joinpath(f"{voice_ref_audio}"))
...
model_cfg = OmegaConf.load(
    args.model_cfg or config.get("model_cfg", str(files("echoforge_tts").joinpath(f"configs/{model}.yaml")))
)
