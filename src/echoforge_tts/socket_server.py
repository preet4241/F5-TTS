...
class TTSStreamingProcessor:
    def __init__(self, model, ckpt_file, vocab_file, ref_audio, ref_text, device=None, dtype=torch.float32):
        ...
        model_cfg = OmegaConf.load(str(files("echoforge_tts").joinpath(f"configs/{model}.yaml")))
        self.model_cls = get_class(f"echoforge_tts.model.{model_cfg.model.backbone}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ref_audio",
        default=str(files("echoforge_tts").joinpath("infer/examples/basic/basic_ref_en.wav")),
