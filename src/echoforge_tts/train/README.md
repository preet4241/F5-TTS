...
Example data processing scripts, and you may tailor your own one along with a Dataset class in `src/echoforge_tts/model/dataset.py`.
...
python src/echoforge_tts/train/datasets/prepare_emilia.py

# Prepare the Wenetspeech4TTS dataset
python src/echoforge_tts/train/datasets/prepare_wenetspeech4tts.py

# Prepare the LibriTTS dataset
python src/echoforge_tts/train/datasets/prepare_libritts.py

# Prepare the LJSpeech dataset
python src/echoforge_tts/train/datasets/prepare_ljspeech.py
```
...
```bash
python src/echoforge_tts/train/datasets/prepare_csv_wavs.py /path/to/metadata.csv /path/to/output
```
...
# .yaml files are under src/echoforge_tts/configs directory
accelerate launch src/echoforge_tts/train/train.py --config-name EchoForge_v1_Base.yaml

# possible to overwrite accelerate and hydra config
accelerate launch --mixed_precision=fp16 src/echoforge_tts/train/train.py --config-name EchoForge_v1_Base.yaml ++datasets.batch_size_per_gpu=19200
```

### 2. Finetuning practice
Discussion board for Finetuning [#57](https://github.com/SWivid/F5-TTS/discussions/57).

Gradio UI training/finetuning with `src/echoforge_tts/train/finetune_gradio.py` see [#143](https://github.com/SWivid/F5-TTS/discussions/143).
