...
from echoforge_tts.model import CFM, Trainer
from echoforge_tts.model.dataset import load_dataset
from echoforge_tts.model.utils import get_tokenizer


os.chdir(str(files("echoforge_tts").joinpath("../..")))  # change working directory to root of project (local editable)


@hydra.main(version_base="1.3", config_path=str(files("echoforge_tts").joinpath("configs")), config_name=None)
def main(model_cfg):
    model_cls = hydra.utils.get_class(f"echoforge_tts.model.{model_cfg.model.backbone}")
    ...
    trainer = Trainer(
        ...
        checkpoint_path=str(files("echoforge_tts").joinpath(f"../../{model_cfg.ckpts.save_dir}")),
