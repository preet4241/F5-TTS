...
path_data = str(files("echoforge_tts").joinpath("../../data"))
path_project_ckpts = str(files("echoforge_tts").joinpath("../../ckpts"))
file_train = str(files("echoforge_tts").joinpath("train/finetune_cli.py"))
...
def create_metadata(name_project, ch_tokenizer, progress=gr.Progress()):
    ...
    for line in progress.tqdm(data.split("\n"), total=count):
        ...
        text = text.strip()
        text = convert_text_to_phonemes([text])[0]
...
with gr.Blocks() as app:
    gr.Markdown(
        """
# EchoForge AI — Automatic Finetune

This is a local web UI for EchoForge AI finetuning support. This app supports the following TTS models:
